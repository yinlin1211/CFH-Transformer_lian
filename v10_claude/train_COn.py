"""
CFH-Transformer v10_claude 训练脚本（train_COn 变体）
==============================
保存准则：COn_f1 最高时保存 best_model.pt（同 train.py，作为备用入口）

所有修复与 train.py 相同：
  - HarmonicTokenizer 移除 Linear(192→128) 瓶颈
  - NaN guard + GradScaler(init_scale=2^13)
  - CQT padding 修正为 -80.0
  - 最佳模型保存准则：COn_f1
"""
...
def infer_full_song(model, cqt_np, device, infer_chunk=256):
    """50% 重叠推理（与 predict_to_json.py 对齐）。"""
    ...
    cqt_t = torch.from_numpy(cqt_np).float().unsqueeze(0)
    with torch.no_grad():
...
def main():
    ...
    # 数据集（论文对齐：extra_train_splits=['val'] 使用400首训练）
