"""
CFH-Transformer v10_claude 训练脚本
==============================
保存准则：COn_f1 最高时保存 best_model.pt（COn = onset-only F1）

主要修复（相对早期版本）：
  [修复1] HarmonicTokenizer 移除 Linear(192→128) 瓶颈，改用 LayerNorm(192)
  [修复2] H = n_octaves(6) × conv_channels(32) = 192，octave 结构完整保留
  [修复3] 推理时 CQT padding 改为 -80.0（dB 静音值），而非 0.0
  [修复4] NaN guard：loss 非有限时跳过 batch，防止坏梯度污染权重
  [修复5] GradScaler(init_scale=2^13)：防 float16 onset logit 溢出
  [修复6] 最佳模型保存准则改为 COn_f1

调度器：CosineAnnealingLR（论文未提调度器，无 warmup）
"""
