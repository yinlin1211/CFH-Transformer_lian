"""
CFT Model — v9（谐波结构修复版）
========================================

【v9 相对 v7 的核心修复：谐波瓶颈问题】

问题根因（为什么 COnP 停在 ~72% = "wo harmonics" 水平）：
  v7 的 HarmonicTokenizer 在 3D 卷积之后有一个 Linear(6*32=192, 128) 投影。
  这个投影将 6 个 octave 的特征压缩混合成 128 个通用通道，导致：
    - H[0] 是所有 6 个 octave 特征的线性混合，没有 octave 专属性
    - Transformer 无法区分"octave 1 的特征"和"octave 2 的特征"
    - HT Transformer 无法学到"基音在 octave k → 泛音在 octave k+1 同时激活"
    - 效果等同于论文消融实验 "wo harmonics" ≈ 72% COnP

【v9 修复内容】
1. HarmonicTokenizer：移除 Linear(192, 128) 瓶颈投影
   - 改用 LayerNorm(192) 替代（保留训练稳定性，不破坏 octave 结构）
   - H = n_octaves × conv_channels = 6 × 32 = 192（显式 octave 结构保留）
   - 保证 H[0:32]=octave_0特征, H[32:64]=octave_1特征, ..., H[160:192]=octave_5特征

【乐理原理】
  人声谐波序列：基音 f₀ → 2f₀(+1 octave) → 4f₀(+2 octaves) → 8f₀(+3 octaves)
  在 48 bins/octave 的 CQT 中，octave 间距 = +48 bins（固定偏移）
  3D 卷积已捕获跨 octave 的谐波关系，但 Linear(192→128) 瓶颈会丢失
  octave 身份标记，导致 Transformer 无法重建"f₀ + 谐波"的协同激活

【v6 保留修复（全局身份嵌入）】
1. FH Transformer：temporal_embed（论文公式2：ε(t)）
2. HT Transformer：freq_embed（论文公式3：H(f)）
3. TF Transformer：harm_embed（论文公式4：T(h)）
